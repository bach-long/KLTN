...
function Me() {
  ...
  const handleBackward = (e) => {
    setCurrent({
      parentId: e.target.getAttribute("folderid")
        ? parseInt(e.target.getAttribute("folderid"))
        : null,
      level: parseInt(e.target.getAttribute("level")),
    });
    setBreadcrum((prev) => {
      return prev.slice(0, 1 + parseInt(e.target.getAttribute("level")));
    });
  };
  const navbarItems = [
    { name: "Home", path: "/" },
  ...
  const items = [
    {
      label: (
        ...
        >
          <FilePdfOutlined /> Tải lên file pdf
        </span>
      ),
      key: "upload file pdf",
    },
    {
      label: (
        <span
          onClick={() => {
            handleOpenModal(".docx, .doc");
          }}
        >
          <FileWordOutlined /> Tải lên file word
        </span>
      ),
    ...
    },
    {
      label: (
        <span
          onClick={() => {
            handleOpenModal(".xlsx, .xls");
          }}
        >
          <FileExcelOutlined /> Tải lên file excel
        </span>
      ),
    ...
    },
    {
      label: (
        <span
          onClick={() => {
            handleOpenModal(".pptx, .ppt");
          }}
        >
          <FilePptOutlined />
          Tải lên file powerpoint
        </span>
      ),
  ...
  const handleFoward = (folderId, folderName) => {
    setCurrent({
      ...
      level: parseInt(current.level) + 1,
    });
    setBreadcrum((prev) => {
      return [
        ...prev,
    ...
    const getDocs = async () => {
      try {
        const data = await myDocuments(
          current.parentId,
          selectedMenu === "marked" ? true : null,
          selectedMenu === "deleted" ? true : null,
          type,
          date && date.dateString[0] ? date.dateString[0] : null,
          date && date.dateString[1] ? date.dateString[1] : null
        );
