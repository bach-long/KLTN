...
function MovingMenu({
    ...
    if (open) {
      ...
      const getFolders = async () => {
        try {
          ...
          setDocuments(
            data.data.documents.filter((item) => {
              return item.id != id;
            })
          );
  ...
  return (
    <Modal
      ...
      footer={[
        <Button
          disabled={folderId == parentId || (!folderId && !parentId) || loading}
          key="Ok"
          type="primary"
          onClick={async () => {
            setLoading(true);
            await handleMove(id, { parent_id: folderId });
          }}
        >
          Di chuyển
        </Button>,
    ...
    >
      <div className="modal-content">
        <h2>
          ...
          <Button
            onClick={() => {
              setFolderId(current ? current.id : null);
            }}
          >
            <FolderFilled /> {current ? current.name : "Tài liệu của tôi"}
          </Button>
        </h2>
        <h3>
          <ArrowLeftOutlined
            onClick={() => {
              if (parentFolder) {
                setFolderId(parentFolder.parent_id);
              }
            }}
          />{" "}
          {parentFolder ? parentFolder.name : " Tài liệu của tôi"}
        </h3>
            ...
            <>
              <Menu style={{ height: 500, overflow: "auto" }}>
                {documents.map((item) => (
                  ...
                  >
                    <Typography.Text key={item.id} style={{ fontSize: 16 }}>
                      {" "}
