...
router.post('/login', validate.validateLogin(), async (req, res) => {
  try {
    ...
    const token = jwt.sign({id: user.id,...user.dataValues}, process.env.ACCESS_TOKEN_SECRET)
...
router.post('/signup', validate.validateRegisterUser(), async (req, res) => {
  try {
    ...
    const result = await sequelize.transaction(async (t) => {
      const hashToken = bcrypt.hashSync(Math.random().toString(36).substring(2), saltRounds);
      const activeToken = Buffer.from(hashToken).toString('base64')
      .replace(/\+/g, '-')
      .replace(/\//g, '_')
      .replace(/=/g, '');
      ...
      queue.add({to: user.email, subject: "Kích hoạt tài khoản", text: `http://localhost:5000/auth/active/${user.active_token}`})
